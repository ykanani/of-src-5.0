...
template<class Type>
sem2zFvPatchField<Type>::sem2zFvPatchField
(
    const fvPatch& p,
...
sem2zFvPatchField<Type>::sem2zFvPatchField
(
    const sem2zFvPatchField<Type>& ptf,
    const fvPatch& p,
    const DimensionedField<Type, volMesh>& iF,
...
template<class Type>
void sem2zFvPatchField<Type>::autoMap
(
...
template<class Type>
void sem2zFvPatchField<Type>::rmap
(
...
)
{
    ...
    const sem2zFvPatchField<Type>& tiptf =
        refCast<const sem2zFvPatchField<Type> >(ptf);
...
template<class Type>
void sem2zFvPatchField<Type>::updateCoeffs()
{
    if (this->updated())
    {
        return;
	...
	if (curTimeIndex_ == -1 )
	{
		...
		tensorField LundR(sampleRplus.size(), pTraits<tensor>::zero);

		scalarField sampleU(sampleUplus*utau);
		symmTensorField sampleRorg(sampleRplus*utau*utau);
                scalarField sampleY(samplePoints*delta_);
		tensor R(1,0,0,0,0,-1,0,1,0);   // rotation tensor - counterclockwise rotation since we have Lund matrix
                                                // in the imaginary wall coordinate of x'(sreamwise),y'(wall normal),
                                                //                                                 // not x is still streamwise but the wall normal is z
                tensorField sampleR(R&sampleRorg&R.T());
                                                //
		//Info << "sampleRplus = " << sampleRplus[37] << endl;

		//Info << "sampleRorg = " << sampleRorg[37] << endl;
		Info << "sampleR = " << sampleR << endl;
		//Info << "sampleY/delta = " << samplePoints[37] << endl;
	...
	if (curTimeIndex_ != this->db().time().timeIndex()      )
		...
		Info << "here1" << endl;
		tensorField sampleR(R&sampleRorg&R.T());
		//Info << sampleR.component(tensor::XX) << endl;
		//Info << sampleY << endl;
		Lund.replace(tensor::XX, sqrt(sampleR.component(tensor::XX)));
		Lund.replace(tensor::YX, sampleR.component(tensor::XY)/(Lund.component(tensor::XX)+ROOTVSMALL));
    		Lund.replace(tensor::ZX, sampleR.component(tensor::XZ)/(Lund.component(tensor::XX)+ROOTVSMALL));
		Lund.replace(tensor::YY, sqrt(sampleR.component(tensor::YY)-sqr(Lund.component(tensor::YX))));
    		Lund.replace(tensor::ZY, (sampleR.component(tensor::YZ) - Lund.component(tensor::YX)*Lund.component(tensor::ZX) )
					/(Lund.component(tensor::YY)+ROOTVSMALL));
		Lund.replace(tensor::ZZ, sqrt(sampleR.component(tensor::ZZ) - sqr(Lund.component(tensor::ZX))-sqr(Lund.component(tensor::ZY))));
		...
		vector dx(0,0,0);  
	        ...
	        vector UConv(Uinf_);
				...
				// sigmaMin for inside the BL+sigmaMAx and SigmaMAx for outside
				// BL is normal to Zplane
				if ( pp_[i][2] < delta_ - sigmaMax_[2])
				{
					sigma_[i] = sigmaMin_;
				}
				else if ( pp_[i][2] >= delta_ - sigmaMax_[2] && pp_[i][2] <= delta_ + sigmaMax_[2])
				{
					sigma_[i] = sigmaMin_ + (pp_[i][2]-(delta_ - sigmaMax_[2]))/(2*sigmaMax_[2])*(sigmaMax_-sigmaMin_);
				}			        	
				else
				{
					sigma_[i] = sigmaMax_;
				...
				forAll ( c,facei )
				{
					...
					bool cond1(mag(dx[1]) <= 1 &&  mag(dx[2]) <= 1);
					//adding periodic faces
					dx = c[facei]-(pp_[i]-Lcy);
					dx[0]=dx[0]/sigma_[i][0];
					dx[1]=dx[1]/sigma_[i][1];
					dx[2]=dx[2]/sigma_[i][2];
					bool cond2(mag(dx[1]) <= 1 &&  mag(dx[2]) <= 1);
					dx = c[facei]-(pp_[i]+Lcy);
					dx[0]=dx[0]/sigma_[i][0];
					dx[1]=dx[1]/sigma_[i][1];
					dx[2]=dx[2]/sigma_[i][2];
					bool cond3(mag(dx[1]) <= 1 &&  mag(dx[2]) <= 1);
				...
				patchField[facei] = patchField[facei] +
 				...
 				//Ensuring cyclic behaviour
					...
					patchField[facei] = patchField[facei] +
                                ...
                                } //end if outside bb from top
                                else if (ppMinusSigma[1] < bb.min()[1]) //outside bb from bottom
                                {
                                        dx = c[facei]-(pp_[i]+Lcy);
                                        dx[0]=dx[0]/sigma_[i][0];
                                        dx[1]=dx[1]/sigma_[i][1];
		...
		forAll(c,facei)
		{
			...
			{
				if (c[facei][2]<=sampleY[k])
				{
					
					UCoeff=(sampleU[k]-sampleU[k-1])/(sampleY[k]-sampleY[k-1])*(c[facei][2]-sampleY[k-1])+sampleU[k-1];
					LundCoeff=(Lund[k]-Lund[k-1])/(sampleY[k]-sampleY[k-1])*(c[facei][2]-sampleY[k-1])+Lund[k-1];
					//if ( c[facei][2]/delta_ > 0.098 && c[facei][2]/delta_ < 0.102 && c[facei][1]/delta_ > -0.26 && c[facei][1]/delta_ < -0.24)
					//{
					//	Info << "z/delta =  " << c[facei][2]/delta_ << endl;
					//	Info << "facei    =  " << facei << endl;
					//	Info << "UCoef  =  " << UCoeff << endl;
					//	Info << "LundCoef  =  " << LundCoeff << endl;
					//	Info << "Lund      =  " << Lund[37] << endl;
					//	Info << "k=" << k << endl;
					//}
...
template<class Type>
void sem2zFvPatchField<Type>::write(Ostream& os) const
{
