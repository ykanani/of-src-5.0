...
#include "timeVaryingMappedFixedValueRepeatFvPatchField.H"
#include "Time.H"
...
template<class Type>
timeVaryingMappedFixedValueRepeatFvPatchField<Type>::
...
timeVaryingMappedFixedValueRepeatFvPatchField
(
    const timeVaryingMappedFixedValueRepeatFvPatchField<Type>& ptf,
    const fvPatch& p,
    const DimensionedField<Type, volMesh>& iF,
...
:
    ...
    interpolationTime_(ptf.interpolationTime_)
{}
...
template<class Type>
void timeVaryingMappedFixedValueRepeatFvPatchField<Type>::autoMap
(
...
template<class Type>
void timeVaryingMappedFixedValueRepeatFvPatchField<Type>::rmap
(
...
)
{
    ...
    const timeVaryingMappedFixedValueRepeatFvPatchField<Type>& tiptf =
        refCast<const timeVaryingMappedFixedValueRepeatFvPatchField<Type> >(ptf);
...
template<class Type>
void timeVaryingMappedFixedValueRepeatFvPatchField<Type>::checkTable()
{
    // Initialise
    if (mapperPtr_.empty())
    {
        ...
        if (debug)
        {
            Info<< "timeVaryingMappedFixedValueRepeatFvPatchField :"
        ...
        if (debug)
        {
            Info<< "timeVaryingMappedFixedValueRepeatFvPatchField : In directory "
    ...
    // Find appropriate interpolation time in the sampleTimes range
    label lo = -1;
    label hi = -1;
    interpolationTime_ = this->db().time().value() - floor(this->db().time().value() / (sampleTimes_[sampleTimes_.size()-1].value()+SMALL))*sampleTimes_[sampleTimes_.size()-1].value() ;
    //Info << "Interpolation Time = " << interpolationTime_ << endl;
    startSampleTime_ = -1;
    bool foundTime = mapperPtr_().findTime
    (
        sampleTimes_,
        startSampleTime_,
        interpolationTime_,//this->db().time().value(),//
        lo,
        hi
    );
//    Info << "lo = " << lo << endl;
//    Info << "hi = " << hi << endl;
//    Info << "starttimsample_ = " << startSampleTime_ << endl;
//    Info << "endtimsample_ = " << endSampleTime_ << endl; 
//    Info << "end time = " << sampleTimes_[sampleTimes_.size()-1].value() << endl;
 ...
 */  
    if (!foundTime)
    {
        FatalErrorIn
        (
            "timeVaryingMappedFixedValueRepeatFvPatchField<Type>::checkTable()"
...
template<class Type>
void timeVaryingMappedFixedValueRepeatFvPatchField<Type>::updateCoeffs()
{
    if (this->updated())
    {
        return;
...
//    Info << "Interpolation Time = " << interpolationTime_ << endl;
    ...
    else
    {
        ...
        scalar s = (interpolationTime_ - start)/(end - start);
        //Info << "start" << start << endl;
        //Info << "endValue" << end << endl;
        //Info << "s = " << s << endl;
        //                
        if (debug)
        {
	...
	//Info << "startValue" << startSampledValues_[0] << endl;
	//Info << "endValue" << endSampledValues_[0] << endl;
	//Info << "s = " << s << endl; 
    ...
    //const scalar t = interpolationTime_;//this->db().time().timeOutputValue();
...
template<class Type>
void timeVaryingMappedFixedValueRepeatFvPatchField<Type>::write(Ostream& os) const
{
