...
#include "convectiveFvPatchField.H"
...
template<class Type>
Foam::convectiveFvPatchField<Type>::convectiveFvPatchField
(
    const fvPatch& p,
...
Foam::convectiveFvPatchField<Type>::convectiveFvPatchField
(
    const convectiveFvPatchField& ptf,
    const fvPatch& p,
    const DimensionedField<Type, volMesh>& iF,
...
:
    mixedFvPatchField<Type>(p, iF),
    Uc_(dict.lookupOrDefault<scalar>("Uc", 1)),
...
Foam::convectiveFvPatchField<Type>::convectiveFvPatchField
(
    const convectiveFvPatchField& ptpsf
)
:
...
Foam::convectiveFvPatchField<Type>::advectionSpeed() const
{
    ...
    {
        return phip/this->patch().magSf()*this->patch().magSf()/phip*Uc_;
    }
}

template<class Type>
void Foam::convectiveFvPatchField<Type>::updateCoeffs()
{
    if (this->updated())
    {
        return;
    ...
    //Info << "advection speed = " << advectionSpeed() << endl;
...
template<class Type>
void Foam::convectiveFvPatchField<Type>::write(Ostream& os) const
{
    ...
    os.writeKeyword("Uc") << Uc_ << token::END_STATEMENT << nl;
