...
#include "turbInflowFvPatchVectorField.H"
...
turbInflowFvPatchVectorField::turbInflowFvPatchVectorField
(
    const fvPatch& p,
...
turbInflowFvPatchVectorField::turbInflowFvPatchVectorField
(
    const turbInflowFvPatchVectorField& ptf,
    const fvPatch& p,
    const DimensionedField<vector, volMesh>& iF,
...
turbInflowFvPatchVectorField::turbInflowFvPatchVectorField
(
    const turbInflowFvPatchVectorField& fcvpvf,
    const DimensionedField<vector, volMesh>& iF
)
:
...
void turbInflowFvPatchVectorField::updateCoeffs()
{
	if (updated())
	{
		return;
	...
	if (curTimeIndex_ != this->db().time().timeIndex()	)
	{
		...
		forAll ( c,facei ) 
		{
			...
			forAll(pp,i)
			{
				...
				for(int j=0; j<3; j=j+1)
				{
					...
					else
					{
						...
						f = f * sqrt(scalar(5))*exp(-5*mag(dx[j]));
			...
			patchField[facei] = patchField[facei] + Uinf_;
...
// Write
void turbInflowFvPatchVectorField::write(Ostream& os) const
{
...
makePatchTypeField(fvPatchVectorField, turbInflowFvPatchVectorField);
